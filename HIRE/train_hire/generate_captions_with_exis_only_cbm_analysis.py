...
def load_exis_cbm(cbm_path, device):
...
def summarize_existence_trace(trace, existence_key="existence", threshold=0.5):
    scores = []
    probs = []
    sims = []

    for item in trace:
        if existence_key in item["concept_score"]:
            scores.append(item["concept_score"][existence_key])
        if existence_key in item["concept_prob"]:
            probs.append(item["concept_prob"][existence_key])
        if existence_key in item["prototype_similarity"]:
            sims.append(item["prototype_similarity"][existence_key])
    ...
    return {
        ...
        "avg_existence_prob": float(sum(probs) / len(probs)) if probs else 0.0,
        "max_existence_prob": float(max(probs)) if probs else 0.0,
        "avg_existence_proto_sim": float(sum(sims) / len(sims)) if sims else 0.0,
        "max_existence_proto_sim": float(max(sims)) if sims else 0.0,
    }
    ...
    if layer_idx == analysis_layer:
        ...
        for b in range(hidden_states.shape[0]):
            ...
            CBM_TRACE.append({
                ...
                "existence_proto_sim": sim_dict.get("existence", 0.0),
                "safe_proto_sim": sim_dict.get("safe", 0.0),
...
def register_hooks(model, editor, agent, cbm_proto, cbm, inv_label_map, analysis_layer):
    ...
    print(f"Registered HIRE + existence-only CBM hooks to {len(hooks)} layers.")
    return hooks
...
def eval_model(args):
    ...
    cbm_proto, cbm, label_map, inv_label_map = load_exis_cbm(args.cbm_path, device)
    print("Loaded existence-only CBM:", args.cbm_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-file", type=str, default="output/gen_cap_exis_cbm_analysis.jsonl")
