...
def editor_router_layerwise_cbm_hook(
    ...
    # Router는 기존과 동일하게 layer 0의 마지막 토큰 hidden으로 action 결정
    if layer_idx == 0:
    ...
    # 각 layer의 마지막 토큰 hidden 저장
    LAYER_STATES[layer_idx] = hidden_states[:, -1, :].detach().float()

    # 마지막 layer에 도달하면 [B,32,4096] 구성 후 layer-wise CBM 분석
    if layer_idx == num_layers - 1:
        if all(i in LAYER_STATES for i in range(num_layers)):
            ...
            with torch.no_grad():
                _, layerwise_sim, _, _ = cbm_proto(layer_stack)      # [B,L,C]
                concept_logits, concept_score = cbm(layerwise_sim)   # [B,C]
            ...
            for b in range(hidden_states.shape[0]):
                ...
                existence_layer_sim = []
                safe_layer_sim = []
                if "existence" in inv_label_map.values():
                    ex_id = {v: k for k, v in inv_label_map.items()}["existence"]
                    existence_layer_sim = [
                        float(v) for v in layerwise_sim[b, :, ex_id].detach().cpu().tolist()
                    ]
                if "safe" in inv_label_map.values():
                    safe_id = {v: k for k, v in inv_label_map.items()}["safe"]
                    safe_layer_sim = [
        ...
        # 다음 generation step을 위해 layer 저장 초기화
        LAYER_STATES = {}
