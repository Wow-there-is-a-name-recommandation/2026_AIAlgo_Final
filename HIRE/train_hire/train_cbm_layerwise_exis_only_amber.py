...
class LayerWisePrototypeEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = sim.mean(dim=1)  # prototype 자체의 평균 분류 logit [B,C]
...
class LayerWiseConceptBottleneck(nn.Module):
    def __init__(self, num_layers=32, num_concepts=2, hidden_dim=64):
        ...
        input_dim = num_layers * num_concepts
    ...
    def forward(self, layerwise_sim):
        ...
        concept_logits = self.bottleneck(x)     # [B,C]
        concept_score = torch.sigmoid(concept_logits)
        return concept_logits, concept_score


class AmberLayerWiseExistenceDataset(Dataset):
    def __init__(
        self,
        h5_dir,
    ...
    def to_layerwise_feature(self, x):
        x = torch.tensor(x, dtype=torch.float32)

        if x.ndim != 3:
            raise ValueError(f"Expected [L,T,H], got {tuple(x.shape)}")

        # subword 평균만 수행. layer 평균은 하지 않음.
        x = x.mean(dim=1)  # [L,H]
        return x
    ...
    def __getitem__(self, idx):
        ...
        feat = self.to_layerwise_feature(x)  # [L,H]
...
def main(args):
    ...
    num_layers = proto_args.get("num_layers", proto_ckpt.get("num_layers", args.num_layers))

    print("Loaded layer-wise prototype:", args.prototype_checkpoint)
    ...
    dataset = AmberLayerWiseExistenceDataset(
        h5_dir=args.h5_dir,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pbar = tqdm(train_loader, desc=f"LayerCBM Epoch {epoch}/{args.epochs}")
        ...
        for x, y, multi_y in pbar:
            ...
            _, sim, _, _ = proto(x)       # sim: [B,L,C]
            logits, score = cbm(sim)      # logits: [B,C]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out-dir", type=str, default="output/checkpoint/amber_layerwise_cbm_existence_generative")
